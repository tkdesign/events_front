...
create table events_has_curators
(
    ...
    constraint events_has_curators_curator_id_foreign
        foreign key (curator_id) references curators (curator_id)
            on delete cascade,
    constraint events_has_curators_event_id_foreign
        foreign key (event_id) references events (event_id)
            on delete cascade
)
    collate = utf8mb4_unicode_ci;

create index events_has_curators_created_at_index
    on events_has_curators (created_at);

create index events_has_curators_curator_id_index
    on events_has_curators (curator_id);

create index events_has_curators_event_id_index
    on events_has_curators (event_id);

create index events_has_curators_position_index
    on events_has_curators (position);
*/
export default {
  data: () => ({
    itemsPerPage: 10,
