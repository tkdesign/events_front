...
export default {
  ...
  methods: {
    loadItems({page, itemsPerPage, sortBy}) {
      ...
      axios.get('/api/admin/get-lectures-speakers', {params}).then(response => {
    ...
    async deleteItemConfirm() {
      try {
        const response = await axios.delete(`/api/admin/delete-lecture-speaker/${this.editedItem.id}`);
    ...
    async saveForm() {
      ...
      try {
        ...
        let response = null;
        if (tableRowIndex > -1) {
          response = await axios.post(`/api/admin/update-lecture-speaker`, formData, {
            headers: {
          ...
          response = await axios.post(`/api/admin/create-lecture-speaker`, formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
            }
          });
  ...
  created() {
    ...
    axios.get('/api/admin/get-speakers-all').then(response => {
