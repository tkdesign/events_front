...
export default {
  ...
  methods: {
    loadItems({page, itemsPerPage, sortBy}) {
      ...
      axios.get('/api/admin/get-events-users', {params}).then(response => {
    ...
    async deleteItemConfirm() {
      try {
        const response = await axios.delete(`/api/admin/delete-event-user/${this.editedItem.id}`);
    ...
    async saveForm() {
      ...
      try {
        ...
        let response = null;
        if (tableRowIndex > -1) {
          response = await axios.post(`/api/admin/update-event-user`, formData, {
            headers: {
          ...
          response = await axios.post(`/api/admin/create-event-user`, formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
            }
          });
