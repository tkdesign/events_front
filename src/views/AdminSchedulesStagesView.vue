<template>
  ...
  >
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>Schedules/Stages relations table</v-toolbar-title>
...
export default {
  ...
  methods: {
    loadItems({page, itemsPerPage, sortBy}) {
      ...
      axios.get('http://localhost/events/backend/public/api/admin/get-schedules-stages', {params}).then(response => {
    ...
    async deleteItemConfirm() {
      try {
        const response = await axios.delete(`http://localhost/events/backend/public/api/admin/delete-schedule-stage/${this.editedItem.id}`);
    ...
    async saveForm() {
      ...
      try {
        ...
        let response = null;
        if (tableRowIndex > -1) {
          response = await axios.post(`http://localhost/events/backend/public/api/admin/update-schedule-stage`, formData, {
            headers: {
          ...
          response = await axios.post(`http://localhost/events/backend/public/api/admin/create-schedule-stage`, formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
            }
          });
