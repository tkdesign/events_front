...
export default {
  ...
  methods: {
    loadItems({page, itemsPerPage, sortBy}) {
      ...
      axios.get('/api/admin/get-schedules-stages', {params}).then(response => {
    ...
    async deleteItemConfirm() {
      try {
        const response = await axios.delete(`/api/admin/delete-schedule-stage/${this.editedItem.id}`);
    ...
    async saveForm() {
      ...
      try {
        ...
        let response = null;
        if (tableRowIndex > -1) {
          response = await axios.post(`/api/admin/update-schedule-stage`, formData, {
            headers: {
          ...
          response = await axios.post(`/api/admin/create-schedule-stage`, formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
            }
          });
  ...
  created() {
    axios.get('/api/admin/get-schedules-all').then(response => {
      this.scheduleItems = response.data;
    }).catch(error => {
      console.error('Error fetching data:', error);
    });
    axios.get('/api/admin/get-stages-all').then(response => {
